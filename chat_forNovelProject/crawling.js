...
app.get('/crawling', (req, res) => {// http://192.168.133.131:3000/crawling 로 클라이언트가 접속했을 때
    ...
    request({url, encoding: null}, function(error, response, body){
        ...
        // 이미지 주소
        // let imgSrcArr = $('.alt'); //셀렉터로 원하는 요소를 찾는다'
        ...
        // 책 제목
        let titleArr = $('.bookTitle'); //셀렉터로 원하는 요소를 찾는다'
        console.log('title result size:'+titleArr.length);
        for(let i = 0; i < titleArr.length; i++){
            let title = $(titleArr[i]).text();
            resultArr_title.push(title); //원하는 요소를 찾으면, 배열에 넣는다
        }

        // 작가 이름
        let authorArr = $('.authorName'); //셀렉터로 원하는 요소를 찾는다'
        console.log('author result size:'+authorArr.length);
        for(let i = 0; i < authorArr.length; i++){
            resultArr_author.push($(authorArr[i]).text()); //원하는 요소를 찾으면, 배열에 넣는다
            // resultArr_author.push($(authorArr[i]).text().split(' ')[0].toLowerCase()); //원하는 요소를 찾으면, 배열에 넣는다
        }
        ...
        // for(var i=0; i<100; i++){
        //
        //     var email = resultArr_author[i];
        //
        //     connection.query('UPDATE novelProject_storyInfo SET author_email = ? WHERE id = ?', [email, id], function(err, result){
        //
        //     });
        //
        //     id += 1;
