...
app.get('/', (req, res) => { // '/'주소로 클라이언트로부터 GET 요청이 올 때 index.html 파일을 보낸다(응답)
    ...
    if(req.query.room != null){
        ...
        if(name_received='yonju'){
            connection.query('UPDATE chat_tcp SET responded = ? WHERE room_id = ?', ['Y', room_id_received], function(err, result){
...
io.on('connection', (socket) => {
    ...
    if(room_id_received!=''){ //기존에 있던 방에 들어가는 것이라면
        ...
        connection.query('INSERT INTO chat_tcp SET ?', info, function(err, result) {
    ...
    socket.on('disconnect', () => {
        ...
        io.of('/').in(room).clients(function(error,clients){
            ...
            if(clients.length == 0){
                ...
                connection.query('UPDATE chat_tcp SET isEmpty = ? WHERE room_id = ?', ['Y', room], function(err, result){
        ...
        //db에 있는 방 정보를 삭제한다
        // connection.query('DELETE FROM chat_tcp WHERE id = ?', [last_inserted_id], function(err, result) {
